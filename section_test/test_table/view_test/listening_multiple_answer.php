...
<?php 
    ...
    if(isset($_GET['question']))
    {
        ...
        $studentId= $_COOKIE['studentId'];
        $test_no = $row['test_no'];
        $question_no = $row['question_no']+3;
        $user_sql = mysqli_query($conn , "SELECT * from section_user_test WHERE studentId='$studentId' AND test_no='$test_no' AND section='listening' AND type='multiple-answers' AND question_no='$question_no' ");
    ...
    <script type="text/javascript">
        $("#prev").click(function(event) {
            <?php
                if($question_no == 1)
                {
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='listening' and type='summarize-spoken-text' and question_no='3'");
                ...
                else
                {
                    ...
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='listening' and type='multiple-answers' and question_no='$question_prev'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
        ...
        $("#next").click(function(event) {
            <?php
                if($question_no == 2)
                {
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='listening' and type='fill-in-the-blanks' and question_no='1'");
                    $row6= mysqli_fetch_assoc($sql6);  
                    $id= $row6['id'];
                    ?>
                ...
                else
                {
                    ...
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='listening' and type='multiple-answers' and question_no='$question_next'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
                    location.replace("listening_multiple_answer.php?question=<?php echo $id; ?>");
