...
<?php 
    ...
    if(isset($_GET['question']))
    {
        ...
        $studentId= $_COOKIE['studentId'];
        $test_no = $row['test_no'];
        $question_no = $row['question_no']+10;
        $user_sql = mysqli_query($conn , "SELECT * from section_user_test WHERE studentId='$studentId' AND test_no='$test_no' AND section='listening' AND type='single-answer' AND question_no='$question_no' ");
...
<head>
    ...
    <title>Listening: Multiple choice, choose single answers | Origin Overseas - PTE tutors </title>
                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"> <a href="../view_test_table.php?section=listening&testno=<?php echo $row['test_no']; ?>&type=single-answer"> Listening: Multiple choice question Single Answer </a> </li>
                                    ...
                                    <div class="account-content">
                                        <div class="scrollspy-example" data-spy="scroll" data-target="#account-settings-scroll" data-offset="-100">
                                            <div class="row">
                                                ...
                                                <br><br><h4><center><b>Multiple choice question Single Answer</b> : Question <?php echo $question_no; ?></center></h4><br>
                                                    <div class="info">
                                                        ...
                                                        <div class="row" style="margin-top: -45px">
                                                            <div class="col-md-11 mx-auto" align="center">
                                                                <div class="form-group">
                                                                    <textarea rows="3" disabled="true" style="color:black;" cols="108" class="form-control" id="question_text" name="question" required=""><?php echo stripslashes($row['question']); ?></textarea>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
    ...
    <script type="text/javascript">
        $("#prev").click(function(event) {
            <?php
                if($question_no == 1)
                {
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='listening' and type='highlight-correct-summary' and question_no='2'");
                ...
                else
                {
                    ...
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='listening' and type='single-answer' and question_no='$question_prev'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
        ...
        $("#next").click(function(event) {
            <?php
                if($question_no == 2)
                {
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='listening' and type='select-missing-word' and question_no='1'");
                    $row6= mysqli_fetch_assoc($sql6);  
                    $id= $row6['id'];
                    ?>
                    location.replace("listening_select_missing_word.php?question=<?php echo $id; ?>"); <?php
                }
                else
                {
                    $question_next=$row['question_no']+1;
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='listening' and type='single-answer' and question_no='$question_next'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
                    location.replace("listening_single_answer.php?question=<?php echo $id; ?>");
