...
<?php 
    ...
    if(isset($_GET['question']))
    {
        ...
        $studentId= $_COOKIE['studentId'];
        $test_no = $row['test_no'];
        $question_no = $row['question_no']+24;
        $user_sql = mysqli_query($conn , "SELECT * from section_user_test WHERE studentId='$studentId' AND test_no='$test_no' AND section='speaking' AND type='re-tell-lecture' AND question_no='$question_no' ");
...
<head>
    ...
    <title>Speaking: Re Tell Lacture | Origin Overseas - PTE tutors </title>
                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"> <a href="../view_test_table.php?section=speaking&testno=<?php echo $row['test_no']; ?>&type=re-tell-lecture"> Speaking: Re-Tell Lecture </a> </li>
                                        ...
                                        <div class="scrollspy-example" data-spy="scroll" data-target="#account-settings-scroll" data-offset="-100">
                                            <div class="row">
                                                <div class="col-xl-12 col-lg-12 col-md-12 layout-top-spacing"> 
                                                    <br><h4><center><b>Re-Tell Lecture</b> : Question <?php echo $question_no; ?></center></h4>
                                                    <div class="info">
    ...
    <script type="text/javascript">
        $("#prev").click(function(event) {
            <?php
                if($question_no == 1)
                {
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='speaking' and type='describe-image' and question_no='7'");
                ...
                else
                {
                    ...
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='speaking' and type='re-tell-lecture' and question_no='$question_prev'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
                    location.replace("speaking_re_tell_lecture.php?question=<?php echo $id; ?>");
                    <?php
                }
            ?>
        });
        $("#next").click(function(event) {
            <?php
                if($question_no == 4)
                {
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='speaking' and type='answer-short-question' and question_no='1'");
                    $row6= mysqli_fetch_assoc($sql6);  
                    $id= $row6['id'];
                    ?>
                    location.replace("speaking_short_question.php?question=<?php echo $id; ?>"); <?php
                }
                else
                {
                    $question_next=$row['question_no']+1;
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='speaking' and type='re-tell-lecture' and question_no='$question_next'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
                    location.replace("speaking_re_tell_lecture.php?question=<?php echo $id; ?>");
