...
<?php 
    ...
    if(isset($_GET['question']))
    {
        ...
        $studentId= $_COOKIE['studentId'];
        $test_no = $row['test_no'];
        $question_no = $row['question_no']+7;
        $user_sql = mysqli_query($conn , "SELECT * from section_user_test WHERE studentId='$studentId' AND test_no='$test_no' AND section='speaking' AND type='repeat-sentence' AND question_no='$question_no' ");
...
<head>
    ...
    <title>Speaking: Repeat Sentence | Origin Overseas - PTE tutors </title>
                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"> <a href="../view_test_table.php?section=speaking&testno=<?php echo $row['test_no']; ?>&type=repeat-sentence"> Speaking: Repeat Sentence </a> </li>
                                        ...
                                        <div class="scrollspy-example" data-spy="scroll" data-target="#account-settings-scroll" data-offset="-100">
                                            <div class="row">
                                                <div class="col-xl-12 col-lg-12 col-md-12 layout-top-spacing">
                                                    <br><h4><center><b>Repeat Sentence</b> : Question <?php echo $question_no; ?></center></h4><br>
                                                    <div class="info">
                                                        <h6 class="pl-2">
                                                            Audio:</h6>
    ...
    <script type="text/javascript">
        $("#prev").click(function(event) {
            <?php
                if($question_no == 1)
                {
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='speaking' and type='read-aloud' and question_no='7'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
                    location.replace("speaking_read_a_loud.php?question=<?php echo $id; ?>");
                    <?php
                }
                else
                {
                    $question_prev=$row['question_no']-1;
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='speaking' and type='repeat-sentence' and question_no='$question_prev'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
        ...
        $("#next").click(function(event) {
            <?php
                if($question_no == 10)
                {
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='speaking' and type='describe-image' and question_no='1'");
                    $row6= mysqli_fetch_assoc($sql6);  
                    $id= $row6['id'];
                    ?>
                    location.replace("speaking_describe_image.php?question=<?php echo $id; ?>"); <?php
                }
                else
                {
                    $question_next=$row['question_no']+1;
                    $sql6 = mysqli_query($conn , "select id from section_test where test_no='$test_no' and section='speaking' and type='repeat-sentence' and question_no='$question_next'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
                    location.replace("speaking_repeat_sentence.php?question=<?php echo $id; ?>");
