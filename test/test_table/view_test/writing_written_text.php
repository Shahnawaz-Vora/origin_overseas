...
<?php 
    ...
    if(isset($_GET['question']))
    {
        ...
        $studentId= $_COOKIE['studentId'];
        $test_no = $row['test_no'];
        $question_no = $row['question_no']+38;
        $user_sql = mysqli_query($conn , "SELECT * from user_test WHERE studentId='$studentId' AND test_no='$test_no' AND section='writing' AND type='summarize-written-text' AND question_no='$question_no' ");
                    ...
                    <div class="page-header">
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"> <a href="../view_test_table.php?section=writing&testno=<?php echo $row['test_no']; ?>&type=summarize-written-text"> Writing: Summarize Written Text </a> </li>
                                        ...
                                        <div class="scrollspy-example" data-spy="scroll" data-target="#account-settings-scroll" data-offset="-100">
                                            <div class="row">
                                                ...
                                                <div class="col-xl-12 col-lg-12 col-md-12 layout-top-spacing">
                                                    <br><h4><center><b>Summarize Written Text</b> : Question <?php echo $question_no; ?></center></h4><br>
                                                </div>
    ...
    <script type="text/javascript">
        $("#prev").click(function(event) {
            <?php
                if($question_no == 1)
                {
                    $sql6 = mysqli_query($conn , "select id from test where test_no='$test_no' and section='speaking' and type='answer-short-question' and question_no='10'");
                ...
                else
                {
                    ...
                    $sql6 = mysqli_query($conn , "select id from test where test_no='$test_no' and section='writing' and type='summarize-written-text' and question_no='$question_prev'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
        ...
        $("#next").click(function(event) {
            <?php
                if($question_no == 3)
                {
                    $sql6 = mysqli_query($conn , "select id from test where test_no='$test_no' and section='writing' and type='essay' and question_no='1'");
                    $row6= mysqli_fetch_assoc($sql6);  
                    $id= $row6['id'];
                    ?>
                    location.replace("writing_essay.php?question=<?php echo $id; ?>"); <?php
                }
                else
                {
                    $question_next=$row['question_no']+1;
                    $sql6 = mysqli_query($conn , "select id from test where test_no='$test_no' and section='writing' and type='summarize-written-text' and question_no='$question_next'");
                    $row6= mysqli_fetch_assoc($sql6); 
                    $id= $row6['id'];
                    ?>
                    location.replace("writing_written_text.php?question=<?php echo $id; ?>");
