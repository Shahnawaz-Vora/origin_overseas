...
if (!isset($_GET['resume']) && isset($_GET['question']) && isset($_GET['test_no']))
{
    ...
    $sql = "select * from test where test_no = '$test_no' and type='$type' and question_no=1 and section='writing'";
...
if(isset($_GET['resume']) && isset($_GET['question']))
{
    ...
    $question_query = $question-38;
    $sql = "select * from test where test_no = '$test_no' and type='$type' and question_no='$question_query' and section='writing'";
    ?><script type="text/javascript">
        var type= '<?php echo $type; ?>';
        var test_no= '<?php echo $test_no; ?>';
        <?php 
                                    ...
                                    <div class="row ">
                                        <div class="col-xl-6" align="left">
                                            <div style="color: black;font-weight: bolder;font-size: 18px;margin-left: 25px;"  class="">
                                                <button class="mb-2 btn btn-outline-danger  title-text text-dark font-weight-bold" ><span id="question_number">39</span>  of 81</button>
                                            </div>
                                        </div>
        ...
        function pageInc(){
            ...
            if(que >= 39 && que <= 41)
            {
                $.ajax({
                    url: 'ajax.php',
                    type: 'POST',
                    data: {summarize_writing_que: que,type: type,test_no:test_no,answer:answer,remaining_time: remaining_time,word_count:wordCount},
                    success: function (response) {
                        $("#questiontext").remove();
