...
<head>
    ...
    <title>Add Material-Reading: Fill in the Blanks | Origin Overseas - PTE tutors </title>
...
<?php
    if (isset($_POST['submit'])) {
        ...
        for ($i=1; $i <= 8; $i++) {
            ...
            if ($i <= $options) {
                if ($_POST[$index] != "") {
                    array_push($option,addslashes($_POST[$index]));
                    $tot_options +=1;
                }else{
                    array_push($option,'');
                }
        ...
        $sql = mysqli_query($conn, "INSERT INTO material_reading(type,answer, questionText, option1, option2, option3, option4, option5, option6, option7, option8, modify_date) VALUES ('$type','$answer', '$question_text_replaced', '$option[0]', '$option[1]', '$option[2]', '$option[3]', '$option[4]', '$option[5]', '$option[6]', '$option[7]', '$modify_date') ");
        if ($sql == true) {
            ?>
            <script type="text/javascript">    
            ...
            ?>
            <script type="text/javascript">    
            window.location = "reading_fill_in_the_blanks.php?error=1";
            </script>
            <?php
        }    
    }
