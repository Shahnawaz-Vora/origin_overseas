...
<head>
    ...
    <title>Create Test-Listening: Highlight Incorrect Words | Origin Overseas - PTE tutors </title>
...
<?php
    if (isset($_POST['submit'])) {
        ...
        if ($tot_options != $options) {
            ?>
            <script type="text/javascript">    
            window.location = "listening_highlight_incorrect_words.php?question_no=<?php echo $question_no ?>&missing=1";
            </script>
            <?php
        }
        ...
        if (in_array($fileActualExt,$allowed)){
            if ($fileError === 0 ){
                if ($fileSize <  8388608999999){
                    ...
                    $sql = mysqli_query($conn, "INSERT INTO test(test_no,question_no,type, audio, transcript, questionText, option1, option2, option3, option4, option5, option6, option7, option8, modify_date,section) VALUES ('$test_no','$question_no','$type', '$fileNameNew', '$transcript', '$question_text', '$option[0]', '$option[1]', '$option[2]', '$option[3]', '$option[4]', '$option[5]', '$option[6]', '$option[7]', '$modify_date','$section') ");
                    if ($sql == true) {
                        ...
                        if($question_no <= 3)
                        {
                            ?><script>location.replace("listening_highlight_incorrect_words.php?testno=<?php echo $test_no; ?>&question_no=<?php echo $question_no ?>&question=success");</script><?php
                        }
                        else
                        {
                            ?><script>location.replace("listening_write_from_dictation.php?testno=<?php echo $test_no; ?>&question_no=1&question=success");</script><?php    
                        }
                    }
                    else
                    {
                        ?><script type="text/javascript">location.href = "listening_highlight_incorrect_words.php?testno=<?php echo $test_no; ?>&question_no=<?php echo $question_no;?>&error=1";</script><?php
                    }
                }else{
                    ?>
                    <script type="text/javascript">
                    window.location = "listening_highlight_incorrect_words.php?testno=<?php echo $test_no; ?>&question_no=<?php echo $question_no;?>&high=1";
                    </script>
                    <?php
                }
            }else{
                    ?>
                    <script type="text/javascript">
                    window.location = "listening_highlight_incorrect_words.php?testno=<?php echo $test_no; ?>&question_no=<?php echo $question_no;?>&file_error=1";
                    </script>
                    <?php
            }
        }else{
            ?>
            <script type="text/javascript">
            window.location = "listening_highlight_incorrect_words.php?testno=<?php echo $test_no; ?>&question_no=<?php echo $question_no;?>&type=1"; 
            </script>
            <?php
        }
    }
