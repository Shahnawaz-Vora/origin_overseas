...
<head>
    ...
    <title>Create Test-Reading: Multiple-choice, choose multiple answer | Origin Overseas - PTE tutors </title>
...
<?php
    if (isset($_POST['submit'])) {
        ...
        if ($tot_options != $options) {
            ?>
            <script type="text/javascript">    
            window.location = "reading_multiple_answer.php?testno=<?php echo $test_no; ?>&question_no=<?php echo $question_no ?>&missing=1";
            </script>
            <?php
        }
        if($ans == null || $ans == "")
        {
            ...
            $sql = mysqli_query($conn, "INSERT INTO test(test_no,question_no,type,answer,question, questionText, option1, option2, option3, option4, option5, option6, option7, option8, modify_date,section) VALUES ('$test_no','$question_no','$type','$ans','$question', '$question_text', '$option[0]', '$option[1]', '$option[2]', '$option[3]', '$option[4]', '$option[5]', '$option[6]', '$option[7]', '$modify_date','$section')");
            if ($sql == true) {
                ...
                if($question_no <= 2)
                {
                    ?><script>location.replace("reading_multiple_answer.php?testno=<?php echo $test_no; ?>&question_no=<?php echo $question_no ?>&question=success");</script><?php
                }
                else
                {
                    ?><script>location.replace("reading_reorder_paragraph.php?testno=<?php echo $test_no; ?>&question_no=1&question=success");</script><?php    
                }
            }
            else
            {
                ?><script type="text/javascript">location.href = "reading_multiple_answer.php?testno=<?php echo $test_no; ?>&question_no=<?php echo $question_no;?>&error=1";</script><?php
            }
        }
