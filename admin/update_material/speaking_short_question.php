...
<head>
    ...
    <title>Update Materials-Speaking: Answer Short Quesrion | Origin Overseas - PTE tutors </title>
...
<body>
                    ...
                    <div class="page-header">
                        ...
                        <nav class="breadcrumb-one" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><span>Speaking: Answer Short Quesrion</span></li>
                            </ol>
                        </nav>
    ...
    <script>
        ...
        function remove(){
            $("#imgshow").remove();
            $("#removebtn").remove();
            $(".uploadimg").append('<input type="file" id="profileImg" class="dropify" data-default-file="" data-max-file-size="5M" name="profileImg" accept="audio/* "/ required=""><p class="mt-2"><i class="flaticon-cloud-upload mr-1"></i><b style="color: red"> Upload Image</b></p>');
            ...
            if (in_array($fileActualExt,$allowed)){
                if ($fileError === 0 ){
                    if ($fileSize <  8388608999){
                        ...
                        $sql = mysqli_query($conn, "UPDATE material_speaking SET audio='$fileNameNew',answer='$answer',transcript='$transcript',modify_date='$modify_date' WHERE speakingMaterialId='$id'");
                        if ($sql == true) {
                            move_uploaded_file($fileTmpName,$fileDestination);
                            unlink("../../database/audio/".$originalfile);
                            ?>
                            <script type="text/javascript">     
                            window.location = "../manage_material_table.php?section=speaking&type=answer-short-question&update=success"; 
                            </script>
                            <?php    
                        }else{
                            ?>
                            <script type="text/javascript">    
                            window.location = "speaking_short_question.php?question=<?php echo $id ?>&error=1";
                            </script>
                            <?php
                        }
                    }else{
                        ?>
                        <script type="text/javascript">
                        window.location = "speaking_short_question.php?question=<?php echo $id ?>&high=1";
                        </script>
                        <?php
                    }
                }else{
                        ?>
                        <script type="text/javascript">
                        window.location = "speaking_short_question.php?question=<?php echo $id ?>&file_error=1";
                        </script>
                        <?php
                }
            }else{
                ?>
                <script type="text/javascript">
                window.location = "speaking_short_question.php?question=<?php echo $id ?>&type=1"; 
                </script>
                <?php
            }
        }
        else{
            $sql = mysqli_query($conn, "UPDATE material_speaking SET answer='$answer',transcript='$transcript',modify_date='$modify_date' WHERE speakingMaterialId='$id'");
            if ($sql == true) {
                ?>
                <script type="text/javascript">    
                window.location = "../manage_material_table.php?section=speaking&type=answer-short-question&update=success";
