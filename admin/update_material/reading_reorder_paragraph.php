...
<head>
    ...
    <title>Update Materials-Reading: Reorder Paragraph | Origin Overseas - PTE tutors </title>
        ...
        for (var i = 0; i < allTextBoxes.length - 1; i++) {
            if(sorted_arr[i + 1] != ""){
                if (sorted_arr[i + 1] == sorted_arr[i]) {
                    Snackbar.show({ text: "Please enter different value in Answer Order, it can't be same!", duration: 2000, });
                    return false;
                }
            }
        }
...
<?php
    if (isset($_POST['submit'])) {
        ...
        if ($tot_options != $options) {
            ?>
            <script type="text/javascript">    
            window.location.href = "reading_reorder_paragraph.php?question=<?php echo $question_id ?>&missing=1";
            </script>
            <?php
        }
        else{
            
            $sql = mysqli_query($conn, "UPDATE material_reading SET  answer='$answer', option1='$option[0]', option2='$option[1]', option3='$option[2]', option4='$option[3]', option5='$option[4]', option6='$option[5]', option7='$option[6]', option8='$option[7]',  modify_date='$modify_date'  WHERE readingMaterialId='$question_id' ");
            if ($sql == true) {
                ?>
                <script type="text/javascript">    
                window.location = "../manage_material_table.php?section=reading&type=reorder-paragraph&update=success"; 
                </script>
                <?php    
            }else{
                ?>
                <script type="text/javascript">    
                window.location = "reading_reorder_paragraph.php?question=<?php echo $question_id ?>&error=1";
                </script>
                <?php
            }
        }              
