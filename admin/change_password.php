...
<head>
    ...
    <meta name="description" content="Origin Overseas admin Change Password Page">
...
<?php
    if (isset($_POST['submit'])) {
        ...
        $sql = "select * from admin where password = '$opassword' and adminId='$admin_id'";
        $run = mysqli_query($conn,$sql);
        if(mysqli_num_rows($run)>0)
        {
            $sql = "update admin set password = '$password' where adminId='$admin_id'";
            $run = mysqli_query($conn,$sql);
            if($run == true)
            {
