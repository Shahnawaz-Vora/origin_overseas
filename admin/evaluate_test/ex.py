...
conn = mysql.connector.connect(host="localhost",port="3306", user="origin-overseas", passwd="origin-overseas", database="i6130179_wp1")
...
if len(text)== 0:
  ...
  if(sys.argv[4] == "summarize-written-text"):
    ...
    update_cursor.execute("UPDATE user_test SET marks='"+str(marks)+"', content ='"+str(content_marks)+"', vocabulary='"+str(vocab_marks)+"' , grammar='"+str(grammer_marks)+"' , spelling='"+str(spell_marks)+"' , comments='"+comments+"' where test_no ='"+sys.argv[1]+"' and question_no="+sys.argv[2]+" and studentId="+sys.argv[3])
    conn.commit()
  elif(sys.argv[4]== "summarize-spoken-text"):
    total = content_marks+vocab_marks+grammer_marks+spell_marks
    if(total == 0):
      marks=0
    else:
    ...
    update_cursor.execute("UPDATE user_test SET marks='"+str(marks)+"', content ='"+str(content_marks)+"', vocabulary='"+str(vocab_marks)+"' , grammar='"+str(grammer_marks)+"' , spelling='"+str(spell_marks)+"' , comments='"+comments+"' where test_no ='"+sys.argv[1]+"' and question_no="+sys.argv[2]+" and studentId="+sys.argv[3])
    conn.commit()
  elif(sys.argv[4] == "write-from-dictation"):

    user_ans = user_rec[7].split()
    test_ans = rec[8].split()
    length = min(len(user_ans),len(test_ans))
    marks=0
    for z in range(length):
      if(user_ans[z] == test_ans[z]):
        marks += 1
    marks = round(22.5*marks/len(test_ans),3)
    update_cursor.execute("UPDATE user_test SET marks='"+str(marks)+"', comments='"+comments+"' where test_no ='"+sys.argv[1]+"' and question_no="+sys.argv[2]+" and studentId="+sys.argv[3])
    conn.commit()
  pass
  update_cursor.close()
  cursor.close()
