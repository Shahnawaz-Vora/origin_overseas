...
<?php 
    include_once '../database/dbh.inc.php';
    $sql = mysqli_query($conn , "SELECT * from user_test where question_no=81 GROUP BY test_no ");
?>
<!DOCTYPE html>
<html lang="en">
<head>
                                        ...
                                        <table id="alter_pagination" class="table table-hover with-ellipsis" style="width:100%">
                                                ...
                                                <tbody>
                                                    ...
                                                    while($row = mysqli_fetch_assoc($sql)) {
                                                        $t = $row['test_no'];
                                                        $sql3 = mysqli_query($conn,"select * from manage_test where test_no='$t'");
                                                        ...
                                                        echo '<td><div style="overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp: 5;line-height: 1.5;max-height: 5.25;" class="text-center">'.$row3['test_name'].'</div></td>';
                                                    ?>
                                                        ...
                                                        ?>
                                                        <td><div style="overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp: 5;line-height: 1.5;max-height: 5.25;" class="text-center"><?php echo $row['modify_date']; ?></div></td>
                                                        <td class="text-center">
                                                            <button type="button" onclick="editQuestion('<?php echo $test_no; ?>','<?php echo $student_id; ?>');" data-toggle="modal" data-target="#faderightModalEdit" style="border:none;background:none;" class="bs-tooltip" data-toggle="tooltip" data-placement="top" title="" data-original-title=" Edit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>
                                                        </td>
                                                        <td class="text-center">
                                                        <?php
                                                        ...
                                                        ?>
                                                            <button type="button" <?php if($check != 0){echo "disabled";} ?> name="writing" onclick="writing_evaluation('<?php echo $test_no; ?>','<?php echo $student_id; ?>')" class="btn btn-danger">Submit</button>
                                                        </td>
                                                    </tr>
                                                    <?php
                                                        }
    ...
    <script type="text/javascript">
        function writing_evaluation(test_no,studentId){
            $.ajax({
                url: 'ajax.php',
                type: 'post',
                data: {test:test_no,studentId:studentId},
                success: function (response) {
                    ...
                    {
                        Snackbar.show({ text: 'Please Enter Speaking Section Marks First', duration: 4000});
